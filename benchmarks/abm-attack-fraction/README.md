...
Furthermore, the likelihood of an individual to acquire and then transmit the disease is correlated. There is no waning immunity. The benchmark is fitting to an attack fraction of 40\% with a standard deviation of 10\%. There are bounds on $R_0$ > 0; the variance of $R_0$ > 0, and the correlation between acquisition and transmission must lie between 0 and 1 (inclusive).
