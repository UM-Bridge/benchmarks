...
/**
 * This file is an adapted version of the file that implements the
 * benchmark described in README.md. The original version of the
 * implementation is part of the deal.II code gallery, see
 * https://dealii.org/code-gallery.html. There, you can also find
 * Python and Matlab implementations of the same benchmark.
 */
...
/**
 * This is the implementation of the bridge that is part of
 * UMBRIDGE. The `Evaluate()` function implements the forward
 * evaluation for the model. Computing the full posterior probability
 * distribution requires an implementation of the likelihood and prior
...
public:
  ...
  std::vector<std::vector<double>> Evaluate(std::vector<std::vector<double>> const& inputs, [[maybe_unused]] json config) override {
    ...
    static const Vector<double> exact_solution(
