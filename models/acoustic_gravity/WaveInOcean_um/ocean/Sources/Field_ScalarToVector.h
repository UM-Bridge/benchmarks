...
namespace OndoMathX {
    ...
    namespace Field {
        ...
        public:
			...
			// ---------------------------------------------------------------------------------//
			ScalarToVector() = delete;

            ScalarToVector(std::function<std::array<Real,Dim>(const Index&)> aFunc ) : _Func_Point_Data(aFunc)
			{
                assert(Regularity==PointData);
			}

            ScalarToVector(std::function<std::array<Real,Dim>(const RealVector&)> aFunc ) : _Func_C0(aFunc)
			{
                assert(Regularity==C0);
			}
            
            ScalarToVector(std::map<Index,std::array<Real,Dim>> aVectorList) : _Vector_List(aVectorList) 
            {
                 assert(Regularity==Field::Regularity::PiecewiseConstant);
            }
            
            ScalarToVector(std::array<Real,Dim> aVector) : _Vector(aVector)
            {
                assert(Regularity== Field::Regularity::Constant);
        
            }
            
            ScalarToVector(std::function<std::array<Real,Dim>(const RealVector&, const Index&)> aFunc) : _Func_PiecewiseC0 (aFunc)
            {
